...
describe('InMemorySessionRepository', () => {
    ...
    it('updates session status and refresh token hash', async () => {
        ...
        const oldHashResult = await repository.findByRefreshTokenHash(`${TEST_PREFIX}-hash-old`);
        ...
        const newHashResult = await repository.findByRefreshTokenHash(`${TEST_PREFIX}-hash-new`);
    ...
    it('revokes all active sessions for a user', async () => {
        ...
        const find1 = await repository.findByRefreshTokenHash(`${TEST_PREFIX}-hash-1`);
        const find2 = await repository.findByRefreshTokenHash(`${TEST_PREFIX}-hash-2`);
        ...
        const find3 = await repository.findByRefreshTokenHash(`${TEST_PREFIX}-hash-3`);
    ...
    it('creates session with all optional fields', async () => {
        ...
        const findResult = await repository.findByRefreshTokenHash(`${TEST_PREFIX}-hash-full`);
        expect(findResult.success).toBe(true);
        if (findResult.success && findResult.value) {
            expect(findResult.value.status).toBe(SessionStatus.Revoked);
    ...
    it('creates session without optional fields', async () => {
        ...
        const findResult = await repository.findByRefreshTokenHash(`${TEST_PREFIX}-hash-minimal`);
        expect(findResult.success).toBe(true);
        if (findResult.success && findResult.value) {
